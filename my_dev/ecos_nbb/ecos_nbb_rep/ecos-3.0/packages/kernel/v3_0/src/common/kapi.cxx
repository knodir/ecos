...
/*---------------------------------------------------------------------------*/
/* Non-blocking buffers (NBBs)          */

externC void cyg_nbb_create(
    cyg_handle_t        *handle,
    cyg_nbb            *nbb
) __THROW
{
	CYG_TRACE1(1, "inside cyg_nbb_create()", true);
	CYG_ASSERT_SIZES( cyg_nbb, Cyg_NBB );
	
	Cyg_NBB *ret_nbb = new((void *)nbb) Cyg_NBB();

	*handle = (cyg_handle_t)ret_nbb;
}

externC void cyg_nbb_delete(cyg_handle_t mbox) __THROW
{
    //((Cyg_Mbox *)mbox)->~Cyg_Mbox();
}

externC cyg_bool cyg_nbb_put(cyg_handle_t nbb, void *item) __THROW
{
	//CYG_TRACE1(1, "inside cyg_nbb_put()", true);
	cyg_bool res = ((Cyg_NBB *)nbb)->put(item);
	
	//CYG_TRACE1(1, "put_result = %d", res);
	
	return res;
}

externC void * cyg_nbb_get( cyg_handle_t nbb ) __THROW
{
	//CYG_TRACE1(1, "inside cyg_nbb_get()", true);
	void * ref_to_item = NULL;
	/*cyg_nbb_stat_t read_stat = ((Cyg_NBB *)nbb)->get( ref_to_item );
	if (read_stat == READ_DONE){
		CYG_TRACE1(1, "READ_DONE", true);
		CYG_TRACE1(1, "ref_to_item = %d", (int *) ref_to_item );
	}*/
	//cyg_nbb_stat_t read_stat;
	cyg_bool read_stat;
	//ref_to_item = ((Cyg_NBB *)nbb)->get( read_stat );
	ref_to_item = ((Cyg_NBB *)nbb)->get( read_stat );
