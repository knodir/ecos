...
/* cyg_sched_edf_prep() is the first method to call after all threads creation with EDF scheduler.
  Currently this method does two works. First checks whether feasibility of schedule with 
  given number of threads (WCET, period) for example if CPU usage exceeds 100%, 
  obviously it is not possible to run this application. 
  Here application programmer will be prompted about non-feasible schedule 
  and make an appropriate changes to application threads.
  Second task is to calculate threads' execution time based on given [deadline, wcet, period] values.
*/
void cyg_sched_edf_prep() __THROW;
