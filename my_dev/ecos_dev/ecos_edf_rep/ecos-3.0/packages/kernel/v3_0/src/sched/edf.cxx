...
//      sched/edf.cxx
//
//      Earliest Deadline First scheduler class implementation
//
//==========================================================================
...
// Contributors: nodir
// Date:         2009-09-17
// Purpose:      Earliest Deadline First (EDF) scheduler class implementation
// Description:  This file contains the implementations of
//               Cyg_Scheduler_Implementation and
//               Cyg_SchedThread_Implementation.
//               Most of the code in this file is based on MLQ scheduler.
//              
//
//####DESCRIPTIONEND####
//
...
// default priority for EDF thread
cyg_count32 edf_pri;

/* MilliSeconds per systems tick, used to convert from user supplied value 
 from millisecond to system clock tick count */
long ms_per_system_tick;
...
Cyg_Scheduler_Implementation::schedule(void)
{
    ...
    Cyg_Thread *thread = queue->get_head();
    Cyg_Thread *return_thread = queue->get_head();
    
    // start of the EDF code by nodir.qodirov@gmail.com
    
    while ( thread ) {
	    if ((thread->priority == CYG_THREAD_MIN_PRIORITY) || 
		    (thread->priority == CYGNUM_LIBC_MAIN_THREAD_PRIORITY))
	    { } else {
		     
		     if ( thread->deadline_tick_cnt < return_thread->deadline_tick_cnt )
		     {
    ...
    //CYG_TRACE1(1, "queue_map = %b", queue_map);
    
    // end of the EDF code by nodir.qodirov@gmail.com
    
    CYG_INSTRUMENT_MLQ( SCHEDULE, return_thread, index);
    
    CYG_ASSERT( return_thread != NULL , "No threads in run queue");
    CYG_ASSERT( return_thread->queue == NULL , "Runnable thread on a queue!");
    //CYG_TRACE1(1, "return_thread->priority = %b", return_thread->priority);
   
    CYG_REPORT_RETVAL(return_thread);
...
Cyg_Scheduler_Implementation::add_thread(Cyg_Thread *thread)
{
    ...
    //Cyg_SchedThread_Implementation::edf_schedule(thread);
	
    //cyg_priority pri = (cyg_priority) thread->edf_info->deadline;
...
{
#ifdef CYGDBG_KERNEL_TRACE_TIMESLICE_EDF
    CYG_REPORT_FUNCTION();
#endif
...
/* start of the code by nodir.qodirov@gmail.com */
    ...
    /* used to calculate thread's EDF deadline value from its *wcet* and *period* 
	edf_prep() is called only once by cyg_sched_edf_prep()
	declared in kapi.cxx after creation of all of the threads
	application programmer who wants to use EDF scheduler 
	makes this call to this function in his application code 
	Here we do basic initialization for EDF threads */
void Cyg_SchedThread_Implementation::edf_prep()
{
	//CYG_TRACE1(1, "from edf_schedule()", true);
	//CYG_TRACE1(1, "Cyg_Sys_Gen_Info::ms_per_sys_tick = %d", Cyg_Sys_Gen_Info::ms_per_sys_tick);
	Cyg_Thread *thread = Cyg_Thread::get_list_head(); // get_current_thread();
	cyg_count32 sum = 0;
	...
	while ( thread ) {
			...
			(thread->priority == CYGNUM_LIBC_MAIN_THREAD_PRIORITY))
		{
			...
			thread->priority = (cyg_priority) edf_priority();
			
			thread->deadline_tick_cnt = thread->edf_info->deadline/ms_per_system_tick;
			thread->wcet_tick_cnt = thread->edf_info->wcet/ms_per_system_tick;
			thread->period_tick_cnt = thread->edf_info->period/ms_per_system_tick;
			//thread->period_tick_cnt = Cyg_Clock::real_time_clock->current_value() + thread->edf_info->period/ms_per_system_tick;
			
			sum += (100*thread->edf_info->wcet)/thread->edf_info->period;
	...
	if (sum >= 100)
	{
		CYG_TRACE1(1, "CPU usage is  %d %\n", sum);
		CYG_ASSERT( false, "Non-feasible schedule, pls correct thread's wcet/period.\n");
	}
}

cyg_count32 Cyg_SchedThread_Implementation::edf_priority( void )
{
	//CYG_TRACE1(1, "edf_pri = %d ", edf_pri);
	//return edf_pri++;
	return edf_pri;
}

void Cyg_SchedThread_Implementation::edf_init()
{
	edf_pri = 5;
	//Cyg_Sys_Gen_Info();
	
	long rtc_resolution[] = CYGNUM_KERNEL_COUNTERS_RTC_RESOLUTION;
	ms_per_system_tick = 1000/rtc_resolution[1];
	
	//Cyg_Sys_Gen_Info::ms_per_sys_tick = 0;//ms_per_system_tick;
	
	//CYG_TRACE1(1, "This is from edf_init() ", true);
}

cyg_priority Cyg_SchedThread_Implementation::get_main_thread_priority()
{
	return CYGNUM_LIBC_MAIN_THREAD_PRIORITY;
}
