...
Cyg_Thread::resume()
{
    ...
    if( suspend_count == 1 )
    {
	    ...
	    //CYG_TRACE1(1, "cyg_current_time = ", Cyg_Clock::real_time_clock->current_value());
#ifdef CYGSEM_KERNEL_SCHED_EDF	
		if ((this->priority == CYG_THREAD_MIN_PRIORITY) || 
			(this->priority == Cyg_Thread::get_main_thread_priority()))
		{
		} else {
			//long rtc_resolution[] = CYGNUM_KERNEL_COUNTERS_RTC_RESOLUTION;
			//long ms_per_system_tick = 1000/rtc_resolution[1];
			this->deadline_tick_cnt = this->period_tick_cnt - this->wcet_tick_cnt;
			//this->deadline_tick_cnt = (this->edf_info->deadline)/ms_per_system_tick;
			//this->deadline_tick_cnt = Cyg_Clock::real_time_clock->current_value() + (this->edf_info->deadline)/ms_per_system_tick;
		}
#endif
	}
    }
